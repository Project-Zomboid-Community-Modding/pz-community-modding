...
function AUDSpawnVehiclePanel:setVisible(visible)
...
function AUDSpawnVehiclePanel:create()
    ...
    self.ModuleVehsList = {}
    for _, val in pairs(self.ModuleVehs) do
        table.insert(self.ModuleVehsList, val)
    end
    ------------------
    ...
    for i=1, #self.ModuleVehsList do
        self.categoryOptions:addOption(self.ModuleVehsList[i][1]:getModule():getName())
    end
